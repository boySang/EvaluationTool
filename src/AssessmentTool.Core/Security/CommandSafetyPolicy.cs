...
public sealed class CommandSafetyPolicy
{
    ...
    private static readonly Regex AllowedReadOnlyShape = new Regex(
        @"^(?:(?:show|display)\s+(?!clock(?:\s|$))(?:(?:[A-Za-z0-9_-]+\s+){0,3})version(?:\s+[A-Za-z0-9_.-]+)*|(?:show|display)\s+clock|uname(?:\s+-[A-Za-z]+)?|hostname|cat\s+(?:/etc/os-release|/etc/lsb-release|/etc/redhat-release|/etc/debian_version|/proc/version|/proc/cpuinfo|/proc/meminfo)|ps(?:\s+(?:-[A-Za-z]+|[aux]+))*|systemctl\s+list-units(?:\s+--(?:all|no-pager|plain|type=(?:[A-Za-z-]+)|state=(?:[A-Za-z-]+)))*|(?:docker|podman)\s+ps(?:\s+(?:-a|--all|--no-trunc|--quiet|--size))*|Get-ComputerInfo)$",
        RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
